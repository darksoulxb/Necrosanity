...
class StabilityEngine:
    ...
    def overall_stability(self, snapshots: List[StabilitySnapshot]) -> float:
        ...
        Penalty breakdown (max 100):
          - Volatility:  up to -30  (CV-based; noisy sensor = unstable system)
          - Bad trend:        -15   (spiking/rising temps are early warning signs)
          - Warning temp:     -10   (elevated but not yet dangerous)
          - Critical temp:    -30   (near thermal limits; major deduction)
        """
        if not snapshots:
            return 100.0
